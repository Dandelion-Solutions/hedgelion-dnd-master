#!/usr/bin/env python3
"""Generate a campaign scaffold from the local D&D Master release.

Standard-library only. No GitHub access. No base64.
"""
...
def yaml_string(value: str) -> str:
    # JSON quoted strings are valid YAML scalars.
...
def main() -> int:
    ...
    if not source_campaign.is_dir():
        raise RuntimeError(f"CAMPAIGN skeleton not found: {source_campaign}")
    if output.exists():
    ...
    replacements = [
        ...
        ("  base_sha: null", f"  base_sha: {yaml_string(args.engine_sha)}"),
        ("  integrated_tag: null", f"  integrated_tag: {yaml_string(args.engine_tag)}"),
        ("  integrated_main_sha: null", f"  integrated_main_sha: {yaml_string(args.engine_sha)}"),
