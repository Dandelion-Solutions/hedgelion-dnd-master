...
| Exact archival oversize | Story growth/sharding owner + T-ARC | Exact material cannot be truncated/paraphrased for file size. If an admitted exact scope may exceed one indivisible file, bounded exact partition/reconstruction semantics must be owner/schema-defined before admission. | PRE-ADMISSION CORRECTNESS LAW |
