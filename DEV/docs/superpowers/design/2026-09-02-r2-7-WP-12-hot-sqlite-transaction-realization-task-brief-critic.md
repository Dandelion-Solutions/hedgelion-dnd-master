...
| C07 | The Step-1 Source Manifest omitted `GAME/SCHEMA/dnd_storage.schema.yaml`, even though it directly constrains current storage discovery and records runtime-baseline provenance while explicitly separating that baseline from existing-campaign runtime selection. Omitting it would leave a current machine contract outside the Step-2 storage/HOT evidence route. | Added it as `SHIPPED SCHEMA / DIRECT STORAGE DISCOVERY + RUNTIME-PROVENANCE CONTRACT`, with required Step-2 inspection of exact-root discovery, storage-format/runtime provenance and write/selection boundaries. Its schema role is machine-contract evidence only; semantic authority remains with owning storage/runtime architecture and contracts. | SIGNIFICANT | YES - SOURCE-MANIFEST REPAIR |
| C08 | The Step-1 Source Manifest omitted `GAME/TEMPLATE/STORAGE_README.md`, a current shipped storage-template surface that states the human-facing storage-main/per-campaign-branch model. Treating the template as irrelevant could miss a current consumer-facing inconsistency; treating it as normative could create false authority. | Added it as `SHIPPED STORAGE-TEMPLATE SUPPORTING SURFACE`, requiring Step-2 consistency inspection against owning storage/branch/runtime contracts only. No semantic authority is assigned to the template prose. | SIGNIFICANT | YES - SOURCE-MANIFEST REPAIR |
