...
| F03 — profile floors/outcomes lack behavioural proof | R2.3 allocation/outcomes; R2.4 finite caller path; R2.6 estimator | `core-catalog.json` vocabulary only | Runtime-local registered profile must produce required closure before optional material; central conservative estimator chooses only legal representations; caller handles terminal `UNSATISFIABLE` by registered finite path. Test exact floor, degradation, terminal failure and no exact-token dependency. | No durable control schema/catalog change in WP-09; no provider policy. |
...
| MechanicalContext, continuation and resolution trace | deterministic mechanics state/derived trace | role source eligibility, continuity/cognition or broad retrieval context |
