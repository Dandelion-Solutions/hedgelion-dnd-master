    ...
    for item in items:
        try:
            payload = context_runtime.resolve_candidate_basis(
                ...
                exact_load=exact_load_for(item),
            )["payload"]
...
class ExactOwnerLoad:
    def __init__(self, fixtures):
        self._records = {}
        for fixture in fixtures:
            self._records[("context.live_route", (fixture.candidate_id,))] = fixture.live_route.as_mapping()
            self._records[("context.live_source", (fixture.candidate_id,))] = fixture.live_source.as_mapping()
            self._records[("context.live_projection", (fixture.candidate_id,))] = fixture.live_projection
            self._records[("context.player_resolution", (fixture.player_resolution.player_id,))] = fixture.player_resolution

    def load_exact(self, family, identity):
        return self._records[(family, identity)]


def exact_load_for(*items):
    return ExactOwnerLoad(tuple(owner_input(item["candidate_id"]) for item in items))


class OwnerRoutedContextAdmissionTests(unittest.TestCase):
    def test_context_assembly_requires_registered_scope(self):
        with self.assertRaisesRegex(context_runtime.ContextContractError, "role, purpose, subject and recipient"):
    ...
    def test_self_consistent_caller_constructed_owner_chain_cannot_mint_admission(self):
        item = candidate("constructed-chain")
        with self.assertRaisesRegex(context_runtime.ContextContractError, "owner-issued|exact native load"):
            context_runtime.resolve_candidate_basis(
                item,
                owner_input=owner_input("constructed-chain"),
                request=make_request(),
            )

    def test_subject_scope_must_bind_to_native_information_subject(self):
        item = candidate("wrong-subject")
        with self.assertRaisesRegex(context_runtime.ContextContractError, "subject"):
            context_runtime.resolve_candidate_basis(
                item,
                request=make_request(subject_id="actor.other"),
                exact_load=exact_load_for(item),
            )

    def test_forged_flags_do_not_override_an_otherwise_valid_exact_route(self):
        item = candidate("forged-flags", current=True, eligible=True)
        with self.assertRaisesRegex(context_runtime.ContextContractError, "current/eligible"):
            context_runtime.resolve_candidate_basis(
                item,
                request=make_request(),
                exact_load=exact_load_for(item),
    ...
    def test_semantic_verdict_callback_cannot_replace_exact_native_load(self):
        item = candidate("semantic-callback")

        def fake_semantic_callback(_family, _identity):
            return {"current": True, "eligible": True}

        with self.assertRaisesRegex(context_runtime.ContextContractError, "native LIVE load"):
            context_runtime.resolve_candidate_basis(
                item,
                request=make_request(),
                exact_load=fake_semantic_callback,
            )
    ...
    def test_inactive_player_resolution_cannot_admit_a_candidate(self):
        ...
        with self.assertRaisesRegex(context_runtime.ContextContractError, "PLAYER resolution"):
            context_runtime.resolve_candidate_basis(
                item,
                request=make_request(),
                exact_load=ExactOwnerLoad((owner_input("inactive-player", player_status="inactive"),)),
            )

    def test_physical_co_presence_does_not_widen_recipient_scope(self):
        item = candidate("private")
        with self.assertRaisesRegex(context_runtime.ContextContractError, "recipient|native"):
            context_runtime.resolve_candidate_basis(
                item,
    ...
    def test_mapping_or_subclass_owner_carrier_cannot_mint_admission(self):
        ...
        with self.assertRaisesRegex(context_runtime.ContextContractError, "internal|exact native"):
            context_runtime.ContextOwnerInput(
...
class RequiredPacketClosureTests(unittest.TestCase):
    def test_unregistered_dependency_relation_is_rejected(self):
        ...
        with self.assertRaises(context_runtime.ContextContractError):
            ...
            context_runtime.assemble_context(request, [root], exact_load=exact_load_for(root))
    def test_required_dependency_closure_precedes_optional_allocation(self):
        ...
        result = context_runtime.assemble_context(request, [root, dependency, optional], exact_load=exact_load_for(root, dependency, optional))
...
class ContextAllocationTests(unittest.TestCase):
    ...
    def test_required_floor_cannot_be_evicted_by_optional_material(self):
        ...
        request = make_request(profile_id="profile.narration", allowed_channels=["EXPLICIT_REF"], max_candidates=5, required_ids=["required"], allowed_relations=[], budget=budget_for(required))
        result = context_runtime.assemble_context(request, [optional, required], exact_load=exact_load_for(optional, required))
...
class OptionalRankingTests(unittest.TestCase):
    def test_optional_ranking_is_deterministic_within_remaining_budget(self):
        ...
        result = context_runtime.assemble_context(request, [b, a, c], exact_load=exact_load_for(a, b, c))
...
class RetrospectiveContextTests(unittest.TestCase):
    def test_retrospective_payload_is_explicitly_a_projection(self):
        ...
        result = context_runtime.assemble_context(request, [item], exact_load=exact_load_for(item))
