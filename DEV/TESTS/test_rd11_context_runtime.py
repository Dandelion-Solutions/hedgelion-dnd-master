...
def request(**values):
    values.setdefault("profile_id", "profile.narrator")
    values.setdefault("role", "NARRATOR")
    values.setdefault("purpose", "narration")
...
def native_runtime_for(items):
    ...
    return context_runtime.bind_context_runtime(repository)
...
class HostBoundContextContractTests(unittest.TestCase):
    def test_context_runtime_is_bound_to_the_trusted_repository_owner(self):
        runtime = context_runtime.bind_context_runtime(RepositoryFixture())
    ...
    def test_registered_role_purpose_subject_recipient_bindings_are_finite(self):
        ...
        bindings = (
            ...
            ("profile.actor", "ACTOR", "assessment"),
            ("profile.story", "CHRONICLER", "story"),
            ("profile.narrator", "NARRATOR", "narration"),
        )
    ...
    def test_campaign_record_is_reloaded_from_pinned_repository_not_candidate_payload(self):
        ...
        runtime = context_runtime.bind_context_runtime(repository)
        result = runtime.assemble(
            bound_request(),
    ...
    def test_live_candidate_uses_only_the_bound_read_capability_and_route(self):
        ...
        runtime = context_runtime.bind_context_runtime(
            RepositoryFixture(),
            live_route=route,
    ...
    def test_fixed_dispatch_revalidates_player_knowledge_and_disclosure_owners(self):
        ...
        result = context_runtime.bind_context_runtime(repository).assemble(
...
class RequiredPacketClosureTests(unittest.TestCase):
    def test_unregistered_dependency_relation_is_rejected(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=["root"], allowed_relations=[], budget=1000)
        with self.assertRaises(context_runtime.ContextContractError):
    ...
    def test_required_dependency_closure_precedes_optional_allocation(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=["root"], allowed_relations=["requires"], budget=budget_for(root, dependency) + 1)
    ...
    def test_missing_required_closure_is_terminal_unsatisfiable(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=["root"], allowed_relations=["requires"], budget=budget_for(root))
    ...
    def test_ineligible_required_closure_is_terminal_unsatisfiable(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=["required"], allowed_relations=[], budget=1000)
...
class ContextAllocationTests(unittest.TestCase):
    ...
    def test_required_floor_cannot_be_evicted_by_optional_material(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=["required"], allowed_relations=[], budget=budget_for(required))
...
class OptionalRankingTests(unittest.TestCase):
    def test_optional_ranking_is_deterministic_within_remaining_budget(self):
        ...
        request_value = request(profile_id="profile.narrator", required_ids=[], allowed_relations=[], budget=budget_for(a, b))
...
class RetrospectiveContextTests(unittest.TestCase):
    def test_retrospective_payload_is_explicitly_a_projection(self):
        ...
        request_value = request(profile_id="profile.chronicler", role="CHRONICLER", purpose="chronicle", allowed_channels=["HISTORY_HINT"], max_candidates=2, required_ids=[], allowed_relations=[], budget=budget_for(item), retrospective=True)
...
class ContextResultTraceTests(unittest.TestCase):
    def test_trace_is_diagnostic_and_does_not_include_payload(self):
        item = candidate("a")
        request_value = request(profile_id="profile.narrator", required_ids=[], allowed_relations=[], budget=budget_for(item))
        result = context_runtime.assemble_context(request_value, [item], runtime=native_runtime_for([item]))
