...
|---|---|---|---|---|---|
| `PO-001` | REQUIREMENT | INCORPORATED | production implementation is active; retrospective/history realization is flowing through current Context/Story/Commentator tasks, with remaining shipped/product consumers gated by the six-wave execution plan | accepted gameplay/navigation owner decision; WP-19 canonical spec; current W04 Context/Story consumers; later W05 shipped integration | NONE |
| `PO-002` | REQUIREMENT | INCORPORATED | production implementation is active; the accepted save/session/menu law remains a required product-path consumer, with final session/bootstrap/product realization routed to Wave-05 rather than deferred behind planning | accepted gameplay/navigation owner decision; WP-19 canonical spec; current durability/session producers; W05 product/bootstrap/session consumers | NONE |
| `PO-003` | REQUIREMENT | INCORPORATED | implementation is active: native history/T0/Story realization through W04.T07A-T07C is accepted, T07D is authorized, and remaining integration/performance/shipped consumers stay on the current Wave-04/05/06 route | historical Actor decision-basis owner decision; accepted T07A-T07C; current T07D/T07-INTEGRATION; performance/shipped proof consumers | NONE |
| `PO-004` | COMPATIBILITY POLICY | INCORPORATED | WP-20 final Senior PASS remains the accepted v1 clean-slate horizon; production implementation is active, task-local schema/version changes may replace unreleased shapes without legacy migration, and release/migration execution remains separately gated | v1 clean-slate compatibility owner decision; current Version Impact gates; active implementation; later release/migration acceptance | NONE |
| `PO-005` | AUTHORITY / SECURITY POLICY | INCORPORATED | creator-login fail-closed authority is fixed and implementation is active; accepted principal/access/LIVE work preserves login-facing selection versus stable-ID PLAYER binding, with remaining bootstrap/product consumers gated downstream | creator-login continuity owner decision; accepted W03 access/currentness; W04 collaboration consumers; W05 bootstrap/product paths | NONE |
...
| `PO-008` | FAILURE / DEGRADATION / DURABILITY-RISK PRODUCT DIRECTION | INCORPORATED | implementation is active under owner-local failure/durability contracts; generic proof and shipped integration proceed through Waves 04-06, while production-like empirical host calibration remains trigger-gated | accepted WP-25 owner-direction; Step 5.5/WP-13 durability owners; current implementation/proof package; later empirical host acceptance | NONE |
| `PO-009` | COMMENTATOR / STORY CORPUS SUFFICIENCY | INCORPORATED | implementation is active: Wave-04 T07A-T07C are accepted, T07D is authorized, and T07-INTEGRATION/Wave-05 shipped consumers remain gated by the current execution plan | Story producer/source contracts; historical Actor basis; Step-4 knowledge/disclosure/access; accepted T07A-T07C; current T07D/T07E/T07-INTEGRATION; later Wave-05 shipped integration | NONE |
