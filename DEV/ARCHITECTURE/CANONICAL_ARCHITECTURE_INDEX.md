...
| Player-facing response language | PO-011 response-language owner | transient `ResolvedResponseLanguage` basis carried only as needed for the current human-visible response | Narrator/turn/emission binding; optional language-quality policy | persistent PLAYER language inferred without owner; missing-policy fallback to another visible language; internal/diagnostic language as ordinary Master prose | `2026-09-26-player-facing-response-language-owner-decision.md` + Step 4/5.12 |
