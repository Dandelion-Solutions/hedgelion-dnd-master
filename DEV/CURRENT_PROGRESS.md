...
3. durable Project Map / canonical-index WP-25 routing;
4. Category-B module version projections implicated by those material CORE changes.

Still deferred behind later explicit implementation/empirical authorization:

```text
focus-scoped FailureDisposition evaluator/adapters
exact common FailureDisposition machine type/enum representation
new persisted failure schema/registry (not required by current architecture)
global health state / generic ACL or operation registry (rejected as baseline)
universal retry engine (rejected as baseline)
...
broader cross-owner failure/cascade/scope-isolation machine realization
```

## Version Impact — bounded recovery
