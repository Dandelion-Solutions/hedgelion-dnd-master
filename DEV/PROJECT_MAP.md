...
- `research/2026-08-27-s6d-09-domain-rules-coverage-matrix-research-architecture-draft.md`, the Step-3 owner decision, collaborative/candidate specs, Step-6 review, Step-7 gate and Step-8 canonicalization — complete S6D-09 evidence chain for Decision C: Minimal Playable Gameplay Spine; whole-project critic PASS.
- `specs/2026-08-27-s6d-10-campaign-rulings-house-rule-boundary-integration-task-brief.md` and matching brief critic, research/decision/collaborative/candidate/adversarial/resolution/canonicalization chain — S6D-10 Steps 1–8 complete; final whole-project critic PASS at 0/0/0. `DEV/ARCHITECTURE/HOUSE_RULES_MECHANICAL_BOUNDARY.md`, its machine ledger/schema/validator/tests and affected-owner amendments own the exact typed adjudication/policy-basis integration. Runtime package selection remains S6D-11-gated.
- `specs/2026-08-28-s6d-11-tests-machine-contract-closure-task-brief.md` and matching `-brief-critic.md` — S6D-11 Step-1 framing plus Senior-audit repair: exact manifest/lock/builder/loader realization, non-self-referential package digests, item-level retirement/demotion/migration of transitional aggregate identity, fail-closed adopted-set/candidate-set semantic compatibility comparison, ResolvedCatalogContext reconstruction, finite S6D-01…10 machine/test equality, closed load-failure reasons and honest package activation; renewed whole-project critic PASS at 0/0/0. Step 2 is not started.
- `DEV/ARCHITECTURE/DOMAIN_RULES_COVERAGE.md`, `CATALOG/domain-rules-coverage.json`, `CATALOG/product-promise-evidence.json`, gameplay-spine package member, Procedure/transition/event schemas, validators and focused tests — canonical S6D-09 owner and machine proof.
...
Route S6D-08 through `DEV/ARCHITECTURE/HEALTH_EFFECTS_RECOVERY.md`, then its complete Steps 1–8 evidence chain, exact Step-2 Resource/HP/LifeState/Effect/Condition owners, Step-3 execution, Step-5.2/5.3/5.7/5.9/5.13 continuity and recovery owners, S6D-03…07 contracts, and the identity-bound health-effects-recovery seed/schema/validator/tests. S6D-08 is canonically closed after senior-audit repair. Reference Effect execution uses canonical `world.effect` envelopes; stable episode identity is envelope-owned, matching is derived from canonical fields, and flat pseudo-state is rejected. Preserve its bounded per-Actor/per-effect authority: no generic scheduler, global scan, cross-domain RestPolicy mutation, Effect-as-Actor-field, generic HP Resource, or unreviewed primitive activation.

## 8.9 Domain rules coverage

Route S6D-09 through `DEV/ARCHITECTURE/DOMAIN_RULES_COVERAGE.md`, its complete Step-1…8 evidence chain and strict machine companions. Decision C supports a minimal playable gameplay spine, not a broad D&D corpus: generic check/save, exact minimal Procedure/movement, exact TargetSpec/AreaSpec applicability through seven bounded `fiction.target_reachable` consumers, current canonical durable-location transitions, significant Asset transfer/equip/use, domain lowering and exact S6D-07/08 routes. The Senior-audit spatial conformance repair is part of this canonical closure. Product keys are atomic reusable mechanic/capability/consequence contracts, never player utterances. Compatibility metadata is not activation. Mechanical-Null has no authoritative world mutation but preserves the exact event/receipt required by its route; every authoritative mutation has its MechanicalEvent. Contest/reaction/broad defense/concentration/economy/crafting/downtime/teleportation/zone creation and broad corpora remain absent/nonselectable.

## 8.10 Campaign rulings / House-Rules mechanical integration

Route S6D-10 through `DEV/ARCHITECTURE/CAMPAIGN_HOUSE_RULES.md`, its repaired owner-decision/canonicalization chain, `ACCESS_CONTROL.md`, `GAME/CORE/ADJUDICATION.md`, Step-4 information eligibility plus the single-context amendment, R2.3/R2.4 context/instruction owners, Step-5.6/5.7/5.8 publication/recovery/currentness, Step-3 accepted-work contracts and the active S6D-04…09 exact consumers. Read the S6D-09 spatial repair Step-6/7/8 chain before deriving active `fiction.target_reachable` consumers. Conformance-only policy realization fixtures remain nonselectable and do not prove a current adopted campaign policy. S6D-10 integrates existing owners; it does not create prose execution, a policy DSL, generic homebrew authoring or dormant vocabulary activation.

## 8.11 S6D integrated tests and machine-contract closure

Route S6D-11 through `DEV/docs/superpowers/specs/2026-08-28-s6d-11-tests-machine-contract-closure-task-brief.md` and its whole-project brief critic, then `RULESET_PACKAGE_IDENTITY.md`, `CATALOG_RESOLUTION.md`, `CATALOG_ADMISSION.md`, the exact S6D-03…10 owners/machine contracts/package members, Step-2/3/5 execution and recovery contracts, and bootstrap/update/access/release-build owners. The ruleset manifest declares exact semantic paths including itself but stores no self-referential package digest; builder-derived evidence computes member/package hashes and the resolved lock records exact snapshot identities/dependency edges. Preserve the accepted same-version update split: a compatible/additive changed ruleset set in a proven forward same-engine-version descendant may be used silently, including by a non-creator, while creator authority is required for coherent pointer persistence and incompatible/ambiguous adoption. The built-in package remains selection-blocked until S6D-11 Steps 2–8 prove the activation gate. Before activation, route every transitional `content_file` / member hash / aggregate `content_set_sha256` carrier and consumer through `TRANSITIONAL_PACKAGE_IDENTITY_KEYS` to removal, strict nonauthoritative derivation, canonical package-snapshot identity or canonical resolved-set identity; no parallel authoritative snapshot chain may remain. A changed same-version set is silent-use eligible only after an exact fail-closed comparison between the adopted and candidate sets proves compatible/additive from current-owner machine evidence; ancestry, compatibility/catalog labels and candidate-load PASS are insufficient, and missing proof blocks use. Do not route S6D-11 through a broad runtime implementation, full-corpus expansion, migration for nonexistent campaigns or S6D-12 final adversarial closure.
