...
Route S6D-09 through `DEV/ARCHITECTURE/DOMAIN_RULES_COVERAGE.md`, its complete Step-1…8 evidence chain and strict machine companions. Decision C supports a minimal playable gameplay spine, not a broad D&D corpus: generic check/save, exact minimal Procedure/movement, bounded engine-owned spatial references, significant Asset transfer/equip/use, domain lowering and exact S6D-07/08 routes. Product keys are atomic reusable mechanic/capability/consequence contracts, never player utterances. Compatibility metadata is not activation. Mechanical-Null has no authoritative world mutation but preserves the exact event/receipt required by its route; every authoritative mutation has its MechanicalEvent. Contest/reaction/broad defense/concentration/economy/crafting/downtime/teleportation/zone creation and broad corpora remain absent/nonselectable.
