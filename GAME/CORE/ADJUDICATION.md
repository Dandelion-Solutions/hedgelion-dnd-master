...
1. use current applicable campaign rulings/house rules first;
...
A lawful Master may always make the bounded situational ruling needed for the current decision when existing rules leave a semantic gap. **Live adjudication authority is not campaign policy-adoption authority.** The current ruling's accepted gameplay consequence follows normal owners even when the ruling itself remains one-off.

If a consequential reusable precedent should become future campaign policy, adopt/persist it only through `CAMPAIGN/RULES/HOUSE_RULES.md` semantics and the current policy-adoption authority/currentness contract. Repetition or local memory alone does not promote a ruling.
...
When the Master supplies a bounded semantic input such as DC/classification/selection to a deterministic consumer, that accepted input is fixed for the Resolution generation. Retry, narration failure or later policy publication does not silently recalculate it.
...
Use only information eligible to the concrete adjudication consumer. DM access to objective truth does not let an NPC social ruling consume facts the NPC does not know/believe or that its owner does not expose to that decision.

## Temporary rulings

A quick local ruling may be internally marked temporary when exact mechanics are uncertain. It does NOT trigger an automatic later internet lookup and does not need campaign-wide adoption before current play can continue.

Verify externally only if the user explicitly asks. If an authorized principal deliberately adopts the ruling as reusable campaign policy, persist it through the House-Rules policy contract; otherwise leave the ruling local while preserving its already-accepted gameplay consequences through their normal owners.
