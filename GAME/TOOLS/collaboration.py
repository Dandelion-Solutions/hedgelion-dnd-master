...
from .mechanics import ExecutionContractError, validate_native_ordering_owner
...
def _ordered_owner(refs: Sequence[NativeBasisRef], owners: Sequence[Mapping[str, object]]) -> str | None:
    for ref, owner in zip(refs, owners):
        if ref.family not in _ORDERED_OWNER_FAMILIES:
            continue
        try:
            ordered = validate_native_ordering_owner(ref.family, owner)
        except (ExecutionContractError, TypeError, ValueError) as exc:
            raise CollaborationAdmissionError(
                "nominated native ordered owner is rejected by its owner schema"
            ) from exc
        if ordered:
            return ref.family
