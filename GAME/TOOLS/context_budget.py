...
def estimate_size(value: Any) -> int:
    ...
    try:
        ...
        raise ContextBudgetError("payload is not a valid canonical context value") from exc


def _candidate_size(candidate: dict[str, Any]) -> int:
    if not isinstance(candidate, dict) or "size" in candidate or "payload" not in candidate:
        raise ContextBudgetError("candidate must carry a payload, not a caller-supplied size")
