...
Provisional PC/incomplete mechanics + resumable onboarding scene. Pass: save structured setup state; status remains initializing.

## S18 — Active requires READY_PC + PLAY_READY
Pass: planned active/provisional tree fails local completeness assertion.

## S19 — Stop unfinished setup is not paused
Pass: save-and-stop retains initializing; paused requires prior PLAY_READY.

## S20 — Unrelated templates are byte-preserved
Pass: README guide/HOUSE_RULES/unrelated YAML are absent from delta unless semantically dirty.
