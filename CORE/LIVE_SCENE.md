...
The campaign branch remains durable long-term canon. While a live epoch is active, the live branch is the authoritative current frontier for the mutable state of that scene. At the end of the epoch, its final state is compacted into one normal campaign persistence batch.
...
Do not trim a knowledge-relevant event merely because another session has not polled recently. If the live file grows beyond a practical hot-state budget, close/compact the epoch and immediately open a fresh epoch if the shared scene continues.
...
A rare event that genuinely affects multiple active live scenes is a slow-path synchronization boundary: freeze/close the affected epochs, resolve the shared event against the reconciled durable state, then reopen live epochs as needed. Do not build distributed multi-branch transactions into the normal turn path.
...
Pure OOC/rules discussion or another response that cannot depend on changing shared-scene state may skip the live probe.

Do not probe the durable campaign branch on every live-scene turn. Query it only when the action depends on world state outside the epoch, during opening/closing, explicit resync/repair, or another existing campaign synchronization boundary.
...
If it changes shared state, creates a new interactive fact, or creates information another PC could legitimately observe/use:
...
Closing protocol:
1. synchronize the live state and replace its status `active -> closing` using optimistic write;
2. after `closing` is canonical, accept no new ordinary actions into that epoch;
...
8. publish one campaign commit containing the durable scene/entity/event/knowledge results, clear the scene's active live pointer, and set `last_absorbed_live_head_sha: L`;
9. after the campaign commit succeeds, the old live branch is non-authoritative and may be deleted/cleaned up when ref deletion is available.

If the scene must remain shared after compaction, open a fresh epoch from the new campaign HEAD.

## Idempotency and recovery

`last_absorbed_live_head_sha` prevents duplicate compaction.

If a retry sees that the exact final live HEAD has already been absorbed into the durable scene, do not apply its deltas again; only finish cleanup/routing.

Recovery cases:
- durable scene points to active live branch: resume from that branch;
- durable scene points to `closing`: any authorized participant may resume bounded compaction;
...
Opening, closing, repair and true cross-scene conflicts are allowed to be slower because they are boundaries, not the ordinary rhythm of play.
